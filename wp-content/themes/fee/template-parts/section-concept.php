...
/* --- 背景のキラキラ装飾 ---
   SPは背景素材 intro_bg_sp_no_hearts.png にキラキラが描かれているが、PCは無地の
   グラデーションだけなので同じ雰囲気になるようCSSで配置している。
   写真・本文の左右にできる余白の中だけに置いてあるので文字には重ならない。 */
.cc-spark, .cc-dot { position: absolute; z-index: 0; pointer-events: none; line-height: 1; }
...
/* --- 中央写真（支給素材 intro.jpg） --- */
/* 最適化プラグインがheight属性を付けても、表示幅に合わせて元の縦横比を保つ */
.cc-photo { display: block; width: 48vw; height: auto; margin: 1.8vw auto 0; border-radius: 2.6vw;
  border: 0.5vw solid #f6b7cd; }

/* --- 紹介文（PC/SP共通） --- */
.cc-copy { margin: 4.2vw 0 0; padding: 0 3vw; line-height: 1.8; font-weight: 600;
  font-size: clamp(20px, 2.25vw, 40px); color: #3a3230; }
.cc-skills { display: flex; justify-content: center; gap: clamp(8px, 2vw, 36px);
  color: #e8397f; font-size: 1.3em; font-weight: 700; }
.cc-skills > span { white-space: nowrap; }
.cc-message { display: block; margin-top: .6em; }
.cc-message > span { display: inline-block; }
...
@media (max-width: 768px) {
  /* 背景は四隅にキラキラが入った額縁状の素材。cover だと下側が見切れるため
     100% 100% でセクション全体に収める（縦横比の差は背景が水彩調のため目立たない）。 */
  .cc { font-size: 2vw;
    background: #fbdce8 url('<?php echo $u; ?>/assets/images/intro_bg_sp_no_hearts.png') center top / 100% 100% no-repeat; }
  /* 背景素材側にキラキラが入っているのでCSSの装飾は出さない（二重になる） */
  .cc-spark, .cc-dot { display: none; }
  .cc-photo { width: 65vw; margin-top: 6vw; border-radius: 6vw; border: 1.5vw solid #fff; }
  .cc-copy { margin-top: 7vw; font-size: clamp(16px, 4vw, 24px); }
  .cc-skills { gap: 2vw; font-size: clamp(18px, 4.8vw, 30px); }
...
<section class="cc" id="concept">
  ...
  <!-- 紹介文 -->
  <p class="cc-copy">
    <span class="cc-skills"><span>技術</span> <span>スピード</span> <span>接客</span> <span>デザイン</span> <span>集客</span></span>
    <span class="cc-message"><span>5つを身につけて、</span><span>卒業後すぐ現場へ！</span></span>
  </p>
